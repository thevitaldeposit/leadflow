// Badge marking a lead created from a missed call — an inbound call the owner
// didn't answer where the caller hung up without leaving a voicemail. Orange,
// to read as distinct from the purple Voicemail badge.
export default function MissedCallBadge({ size = 'sm', boxy = false }) {
  const padding = size === 'xs' ? 'px-1.5 py-0.5 text-[10px]' : size === 'sm' ? 'px-2 py-0.5 text-xs' : 'px-3 py-1 text-sm';
  const radius = boxy ? 'rounded-md' : 'rounded-full';
  return (
    <span className={`inline-flex items-center font-medium border ${radius} bg-warning/10 text-warning border-warning/30 ${padding}`}>
      Missed Call
    </span>
  );
}
