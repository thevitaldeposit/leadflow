...
function Card({ title, children }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      {title && <div className="px-5 py-3.5 border-b border-gray-100"><h2 className="text-sm font-bold text-gray-800">{title}</h2></div>}
      {children}
    </div>
  );
}
...
export default function PaymentDetailPage() {
  ...
  if (error || !pay) {
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-10 text-center text-sm text-gray-400 mt-4">{error || 'Payment not found'}</div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-extrabold text-gray-900 leading-tight">{money(pay.amount, pay.currency)}</p>
              <p className="text-sm text-gray-500">{brandLabel(pay.card_brand)}{pay.card_last4 ? ` •••• ${pay.card_last4}` : ''}{pay.wallet ? ` · ${pay.wallet.replace('_', ' ')}` : ''}</p>
            </div>
          </div>
        ...
        {Number(pay.amount_refunded) > 0 && (
          <p className="text-sm text-amber-700 mt-3">{money(pay.amount_refunded, pay.currency)} refunded · {money(refundable, pay.currency)} refundable</p>
        )}
        {flash && (
          <p className={`text-sm mt-3 ${flash.type === 'ok' ? 'text-emerald-600' : 'text-red-500'}`}>{flash.msg}</p>
        )}
      </div>
          ...
          <ul className="divide-y divide-gray-50">
            {pay.refunds.map((r) => (
              <li key={r.id} className="px-5 py-3 flex items-center gap-3 text-sm">
                ...
                <span className="text-gray-700 flex-1">{money(r.amount, pay.currency)} refunded{r.status && r.status !== 'succeeded' ? ` (${r.status})` : ''}</span>
                <span className="text-xs text-gray-400">{fmtDateTime(r.created)}</span>
              </li>
            ))}
          </ul>
        </Card>
        ...
        <div className="px-5 py-4">
          {pay.status === 'refunded' ? (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <CheckCircle2 size={16} className="text-gray-400" /> This payment has been fully refunded.
            </div>
          ) : !refundOpen ? (
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <p className="text-sm text-gray-500">{money(refundable, pay.currency)} available to refund.</p>
              <button
                onClick={() => { setRefundOpen(true); setFlash(null); }}
                disabled={!canRefund}
                className="flex items-center gap-1.5 text-sm font-medium text-red-600 bg-red-50 hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed px-3 py-2 rounded-lg"
              >
            ...
            <div className="space-y-3">
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 text-sm text-gray-700">
                  ...
                  Full refund <span className="text-gray-400">({money(refundable, pay.currency)})</span>
                </label>
                <label className="flex items-center gap-2 text-sm text-gray-700">
                ...
                <div>
                  <div className="relative w-40">
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      className="w-full text-sm border border-gray-200 rounded-lg pl-6 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent"
                    />
                  </div>
                  <p className="text-[11px] text-gray-400 mt-1">Max {money(refundable, pay.currency)}</p>
                </div>
              )}

              <div className="flex items-center gap-2 pt-1">
                <button onClick={submitRefund} disabled={busy} className="flex items-center gap-1.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 px-4 py-2 rounded-lg">
                  <RotateCcw size={14} /> {busy ? 'Refunding…' : 'Issue refund'}
                </button>
                <button onClick={() => { setRefundOpen(false); setPartial(''); setMode('full'); setFlash(null); }} disabled={busy} className="text-sm text-gray-500 hover:text-gray-700 px-3 py-2 rounded-lg">
                  Cancel
                </button>
              </div>
            </div>
      ...
      {pay.receipt_url && (
        <a href={pay.receipt_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-accent">
