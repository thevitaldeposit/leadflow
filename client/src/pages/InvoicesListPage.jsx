    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 p-4 pt-24" onClick={onClose}>
      <div className="bg-surface rounded-xl shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-3.5 border-b border-divider flex items-center justify-between">
          <h2 className="text-sm font-bold text-content">New invoice — choose a customer</h2>
          <button onClick={onClose} className="text-muted hover:text-muted"><X size={16} /></button>
        </div>
        <div className="p-4">
          <div className="relative mb-3">
            <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted" />
            <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search name, phone, email…" className="w-full text-sm border border-divider rounded-lg pl-8 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent" />
          </div>
          <div className="max-h-72 overflow-y-auto -mx-1">
            {loading ? (
              <div className="py-8 text-center text-sm text-muted">Loading…</div>
            ) : results.length === 0 ? (
              <div className="py-8 text-center text-sm text-muted">No customers found.</div>
            ) : results.map((c) => (
              <button key={c.id} onClick={() => onPick(c)} className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-surface-2 flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-content">{c.display_name}</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 p-4 pt-16 overflow-y-auto" onClick={onClose}>
      <div className="bg-surface rounded-xl shadow-xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-3.5 border-b border-divider flex items-center justify-between">
          <h2 className="text-sm font-bold text-content">Invoice defaults</h2>
          <button onClick={onClose} className="text-muted hover:text-muted"><X size={16} /></button>
        </div>
        {!form ? (
          ...
          <div className="p-5 space-y-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted mt-1.5">Prefills every new invoice. A customer's own contract terms override this when set.</p>
            </div>
            <div className="flex justify-end gap-2 pt-1">
              <button onClick={onClose} className="text-sm text-muted hover:text-content px-3 py-2 rounded-lg">Cancel</button>
              <button onClick={save} disabled={saving} className="flex items-center gap-1.5 text-sm font-medium text-content bg-accent hover:bg-accent/90 disabled:opacity-50 px-4 py-2 rounded-lg"><Check size={14} /> {saving ? 'Saving…' : 'Save defaults'}</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="max-w-5xl mx-auto space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold text-content">Invoices</h1>
          <span className="text-sm text-muted">{invoices.length}</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowDefaults(true)} className="flex items-center gap-1.5 text-sm font-medium text-muted bg-surface-2 hover:bg-surface-2 px-3 py-1.5 rounded-lg"><Settings size={14} /> Defaults</button>
          <button onClick={() => setPicking(true)} className="flex items-center gap-1.5 text-sm font-medium text-content bg-accent hover:bg-accent/90 px-3 py-1.5 rounded-lg"><PlusCircle size={14} /> New Invoice</button>
        </div>
      </div>

      <div className="flex items-center gap-1.5 flex-wrap">
        {FILTERS.map((f) => (
          <button key={f.value} onClick={() => setStatus(f.value)} className={`text-xs font-medium px-3 py-1.5 rounded-full border transition-colors ${status === f.value ? 'bg-well text-content border-divider' : 'bg-surface text-muted border-divider hover:border-divider'}`}>
            {f.label}
          </button>
        ))}
      </div>
        ...
        <div className="bg-surface rounded-xl border border-divider shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-surface-2 border-b border-divider">
              <tr>
                <th className="text-left px-5 py-3 text-xs font-semibold text-muted uppercase tracking-wide">Invoice</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-muted uppercase tracking-wide">Issued</th>
            ...
            <tbody className="divide-y divide-divider">
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-surface-2 cursor-pointer transition-colors" onClick={() => navigate(`/invoices/${inv.id}`)}>
                  ...
                  <td className="px-4 py-3 text-content">{inv.customer_display_name || inv.bill_to_name || '—'}</td>
                  <td className="px-4 py-3"><StatusBadge status={inv.status} /></td>
                  <td className="px-4 py-3 text-right text-content font-medium">{money(inv.total, inv.currency)}</td>
                  <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">{fmtDate(inv.issue_date)}</td>
                  <td className="px-4 py-3 text-xs text-muted whitespace-nowrap">{fmtDate(inv.due_date)}</td>
                </tr>
              ))}
            </tbody>
          </table>
