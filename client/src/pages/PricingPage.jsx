    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="bg-surface rounded-xl border border-divider shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-divider flex items-center justify-between">
          <div className="flex items-center gap-2"><DollarSign size={16} className="text-muted" /><h2 className="text-sm font-bold text-content">Default Price List ({items.length})</h2></div>
          {!addingItem && <button onClick={() => { setAddingItem(true); setEditingItem(null); }} className="flex items-center gap-1.5 text-sm font-medium text-content bg-accent hover:bg-accent/90 px-3 py-1.5 rounded-lg"><PlusCircle size={14} /> Add Price</button>}
        </div>
        {addingItem && <div className="px-5 py-4 border-b border-divider bg-surface-2"><PriceItemForm onSave={saveItem} onCancel={() => setAddingItem(false)} /></div>}
        {items.length === 0 && !addingItem ? (
          <div className="px-5 py-8 text-center text-sm text-muted">No prices yet. Add your standard rates above.</div>
        ) : items.length > 0 && (
          <table className="w-full text-sm">
            <thead className="bg-surface-2 border-b border-divider">
              <tr>
                <th className="text-left px-5 py-2.5 text-xs font-semibold text-muted uppercase tracking-wide">Service Key</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted uppercase tracking-wide">Label</th>
                <th className="text-left px-4 py-2.5 text-xs font-semibold text-muted uppercase tracking-wide">Unit</th>
            ...
            <tbody className="divide-y divide-divider">
              {items.map(it => editingItem === it.id ? (
                <tr key={it.id} className="bg-surface-2"><td colSpan={5} className="px-5 py-4"><PriceItemForm initial={it} onSave={saveItem} onCancel={() => setEditingItem(null)} /></td></tr>
              ) : (
                <tr key={it.id} className="hover:bg-surface-2 transition-colors">
                  <td className="px-5 py-3 font-medium text-content">{it.service_key}</td>
                  <td className="px-4 py-3 text-content">{it.label || '—'}</td>
                  <td className="px-4 py-3 text-muted">{it.unit || '—'}</td>
                  <td className="px-4 py-3 text-content font-semibold">{it.unit_price != null ? `$${it.unit_price}` : '—'}</td>
      ...
      <div className="bg-surface rounded-xl border border-divider shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-divider flex items-center justify-between">
          <div className="flex items-center gap-2"><Percent size={16} className="text-muted" /><h2 className="text-sm font-bold text-content">Discount Groups ({groups.length})</h2></div>
          {!addingGroup && <button onClick={() => { setAddingGroup(true); setEditingGroup(null); }} className="flex items-center gap-1.5 text-sm font-medium text-content bg-accent hover:bg-accent/90 px-3 py-1.5 rounded-lg"><PlusCircle size={14} /> Add Group</button>}
        </div>
